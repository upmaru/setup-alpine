...
apk-tools-url::
URL of the apk-tools static binary to use, or `auto` to select a pinned URL for the current runner architecture.
Custom URLs must end with `#!sha256!` followed by a SHA-256 hash of the file.
This should normally be left at the default value.
+
Default: `auto`

[[arch]] arch::
CPU architecture to install.
When the selected architecture is not natively runnable on the host, the action uses https://www.qemu.org/docs/master/user/main.html[QEMU user space emulator] by default.
...
setup-qemu::
Whether to install and register QEMU when the selected <<arch>> needs emulation on the current runner.
Set this to `false` only when using a native compatible runner, such as `ubuntu-24.04-arm` or `ubuntu-22.04-arm` for `aarch64`; otherwise the action will fail early.
+
Default: `true`
...
=== Set up Alpine aarch64 on a native ARM runner
